...
> 发布有具名镜像标签且演练里做过 A→B→A；备份脚本在，且 2026-09-14 按 runbook 真做过一次恢复（dump 校验和与
> manifest 一致、10 张表行数逐项相等、恢复出的应用以 `200` 提供还原后的上传文件），但两个备份集仍与数据库在同一块
> 物理盘上，异盘副本仍未做。本周期的优先级是把"门禁可信"
